...
public class PatientInfoServiceImpl implements PatientInfoService {
    ...
    public void doDownloadPatientInfo(HttpServletResponse response) throws IOException, NullPointerException {
        ...
        for (PatientInfo p : patientList) {
            ...
            try {
                ...
                datarow.createCell(8).setCellValue(p.getDoctor().getUserId());
    ...
    public String doUploadPateintInfo(MultipartFile file) throws IOException {
        ...
        for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
            ...
            patientInfo.setToday(localDate);
            ...
            Cell cellDelFlag = row.getCell(9);
            patientInfo.setDelFlag((int) cellDelFlag.getNumericCellValue());
            Date a = new Date();
    ...
    private String validExcelFile(MultipartFile file) throws IOException {
        ...
        for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (!(cellPatientPhone.getStringCellValue()).matches("(^&|[0-9,]{9,})")) {
                err += "<br>Error : Row " + i + ", Column " + 5 + "<br>Invalid Patient Phone Number "
                        + cellPatientPhone.getStringCellValue() + ". <br>";
            }
            // doctorId
            Cell cellDocId = row.getCell(7);
            if (cellDocId.getCellType() != Cell.CELL_TYPE_NUMERIC) {
                err += "<br>Error : Row " + i + ", Column " + 7
                        + "<br>Doctor Id has worng data type in this file. <br>";
            }
            if (userDao.dbGetUserById((int) cellDocId.getNumericCellValue()) == null) {
                err += "<br>Error : Row " + i + ", Column " + 7 + "<br>No Doctor matched by the given id. <br>";
            }
            // Status
            Cell cellStatus = row.getCell(8);
            if (cellStatus.getCellType() != Cell.CELL_TYPE_STRING) {
                err += "<br>Error : Row " + i + ", Column " + 8
            ...
            // Del_Flag
            Cell cellDelFlag = row.getCell(9);
            if (cellDelFlag.getCellType() != Cell.CELL_TYPE_NUMERIC) {
                err += "<br>Error : Row " + i + ", Column " + 9 + "<br>Del_Flag has wrong data type in this file. <br>";
            }
            if (cellDelFlag.getNumericCellValue() != 0 && cellDelFlag.getNumericCellValue() != 1) {
                err += "<br>Error : Row " + i + ", Column " + 9
                        + "<br>Del_Flag has wrong data value in this file. <br>";
            }
        }
