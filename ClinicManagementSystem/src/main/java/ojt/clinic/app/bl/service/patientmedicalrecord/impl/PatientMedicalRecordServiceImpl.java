...
public class PatientMedicalRecordServiceImpl implements PatientMedicalRecordService {
    ...
    public String doUploadPatientMedicalRecord(MultipartFile file) throws IOException {
        ...
        for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
            ...
            Cell cellDelFlag = row.getCell(8);
            patientMedicalRecord.setDelFlag((int) cellDelFlag.getNumericCellValue());
            Date a = new Date();
    ...
    private String validExcelFile(MultipartFile file) throws IOException {
        ...
        for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (patient == null) {
                err += "<br>Error : Row " + i + ", Column " + 1 + "<br>No Patient found in this clinic. <br>";
            }
            // Doctor ID
            Cell cellDoctorId = row.getCell(2);
            ...
            User doctor = userDao.dbGetUserById((int) cellDoctorId.getNumericCellValue());
            if (doctor == null) {
                err += "<br>Error : Row " + i + ", Column " + 2 + "<br>No Doctor found in this clinic. <br>";
            }
            // Record Date
            Cell cellRecordDate = row.getCell(3);
            ...
            if (cellMedicalRecord.getCellType() != Cell.CELL_TYPE_STRING) {
                err += "<br>Error : Row " + i + ", Column " + 4
                        + "<br>Medical Record has wrong data type in this file. <br>";
            }
            // Medicine
            Cell cellMedicine = row.getCell(5);
            if (cellMedicine.getCellType() != Cell.CELL_TYPE_STRING) {
                err += "<br>Error : Row " + i + ", Column " + 5 + "<br>Medicine has type in this file. <br>";
            }
            // Cost
            Cell cellCost = row.getCell(6);
            if (cellCost.getCellType() != Cell.CELL_TYPE_NUMERIC) {
                err += "<br>Error : Row " + i + ", Column " + 6 + "<br>Cost has worng data type in this file. <br>";
            }

            // Remark
            Cell cellRemark = row.getCell(7);
            if (cellRemark.getCellType() != Cell.CELL_TYPE_STRING && cellRemark.getCellType() != Cell.CELL_TYPE_BLANK) {
                err += "<br>Error : Row " + i + ", Column " + 7 + "<br>Remark has worng data type in this file. <br>";
            }
            // Del_Flag
            Cell cellDelFlag = row.getCell(8);
            System.out.println("\n\n\n\n\nDel Flag is " + (int) cellDelFlag.getNumericCellValue() + "\n\n\n\n\n");
            if (cellDelFlag.getCellType() != Cell.CELL_TYPE_NUMERIC) {
                err += "<br>Error : Row " + i + ", Column " + 8 + "<br>Del_Flag has wrong data type in this file. <br>";
            }
